---
title: Contributing to Brick
path: /contributing
summary: Contrbuting to brick by proposing changes, reporting bugs and issues
...
- `SKOS.definition`: contains the definition of the class. The value must use the `Literal` constructor from RDFlib:
    ...
    ```
   An alternative location for definitions is `definitions.csv`; we recommend placing text descriptions (with an optional but encourage citation) in the CSV file for automatic incorporation into the ontology definition.
...
3. Install the Python3 dependencies using pip:
    ...
    ```
4. Make the desired changes **to the Python files**. Do not edit the `Brick.ttl` file directly.
5. Compile the `Brick.ttl` file by executing the `generate_brick.py` script using Python
    ```bash
