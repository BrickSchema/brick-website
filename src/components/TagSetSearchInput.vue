<template>
  <div class="relative w-80 mx-auto mt-4 mb-8">
    ...
    <div class="relative w-75">
      <input
        type="text"
        placeholder="Search (example: ZTS)"
        class="bg-background-form border border-gray-500 rounded-full px-4 pl-10 py-2 mt-0 mb-4 outline-none focus:border-primary-500 w-80"
        v-model="query"
        @input="softReset"
        @keyup="performSearch"
    ...
    <transition name="fade">
      <div v-if="query.length > 0 && searchResultsVisible" class="search-results xs:max-h-screen w-full normal-case absolute border left-0 right-0 top-2 text-left mb-4 mt-2 rounded-lg shadow overflow-hidden z-5 overflow-y-auto">
