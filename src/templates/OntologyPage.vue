    ...
    <Layout>
        ...
        <div class="container xs:flex-col sm:flex mx-auto my-16 overflow-x-hidden text-normal">
            ...
            <div class="l-auto pb-4 mx-4 w-80">
                <ontology-search-input :version="$page.ontologyPage.version" />
                ...
                <div class="h-auto">
                    ...
                    <g-link v-if="expanded" v-for="branch in tree" :key="branch.name" class="flex" :to="branch.name">
                        <hierarchy-tree :tree="branch" :hierarchy="[`/ontology/${$page.ontologyPage.version}/#Classes`,`/ontology/${$page.ontologyPage.version}/#Relationships`, ``]"></hierarchy-tree>
                    </g-link>
                </div>
            </div>
            <div class=" l-auto w-full sm:border-l p-4 mx-4 z-5">
                <ul id="breadcrumbs" class="flex overflow-hidden overflow-x-auto">
                    <li
                            v-for="className in $page.ontologyPage.hierarchy"
                            :key="className">
                        <div class="h-100"></div>
                        <g-link
...
<page-query>
    ...
    ontologyPage: ontologyPage (path: $path) {
    title
    content
    version
    subversion
...
<script>
    ...
    export default {
        ...
        metaInfo() {
            return {
                title: this.$page.ontologyPage.title
            }
        },
        created() {
            axios(`${this.$page.metaData.pathPrefix}/ontology/${this.$page.ontologyPage.version}/tree.json?version=${this.$page.metaData.cacheCommitId}`).then(response => {
